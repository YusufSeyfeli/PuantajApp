...
import {WeekTally} from "../../Models/WeekTally";

@Component({
  selector: 'app-weekly-tally',
  templateUrl: './weekly-tally.component.html',
  styleUrls: ['./weekly-tally.component.scss']
})
export class WeeklyTallyComponent implements OnInit {
  WeekDay: any[] = [];
  WeekTally: WeekTally = {};
  ...
  ngOnInit(){
    ...
    this.WeekDay = [
      { label: 'Pazartesi', value: 0, status: "qualified" },
      { label: 'Salı', value: 0, status: "unqualified" },
      { label: 'Çarşamba', value: 0, status: "negotiation" },
      { label: 'Perşembe', value: 0, status: "new" },
      { label: 'Cuma', value: 0, status: "renewal" },
    ]
  }
  navigateToDetail() {
    this.router.navigate(['/comp/students/ozel-puantaj', this.studentId]);
  }
